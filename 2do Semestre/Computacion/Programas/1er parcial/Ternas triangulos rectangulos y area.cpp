/*  FILA F - TEMA 1. Elaborar un programa que:
    Lea dos ternas de números flotantes a1, b1, c1 y a2, b2, c2,
    verifique si cumplen las condiciones para que cada terna
    represente lados de un triángulo, en caso contrario que lo lea
    nuevamente. Luego imprima las respectivas áreas de superficie y
    un mensaje indicando si uno de ellos es un triángulo rectángulo. */
#include<iostream>
#include<math.h>
using namespace std;
int valLados( float a, float b, float c );
float areaTriang( float a, float b, float c );
int verRectangulo( float a, float b, float c );
int main()
...
// areaTriang(): calcula el area de superficie de un triangulo.
    ...
    if ( verRectangulo( a1, b1, c1 ) || verRectangulo( a2, b2, c2 ) )
       cout << "\n\n\tUno de los triángulos dados es rectangulo";
    else
       cout << "\n\n\tNinguno de los triángulos dados es rectangulo";
    cout << "\n\n";
    system( "pause" );
}
int valLados( float a, float b, float c )
